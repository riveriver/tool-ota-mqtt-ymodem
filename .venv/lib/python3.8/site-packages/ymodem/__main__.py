...
from ymodem import __version__
...
class TaskProgressBar:
    ...
    def show(self, task_index, task_name, total, success):
        ...
        print(
            f"\r{task_index} - {task_name} {progress:.2f}% [{a}->{b}]{cost:.2f}s",
            end="",
        )
...
def add_modem_args(parser):
    ...
    parser.add_argument(
        "-pr", "--parity", type=str, default="N", help="Parity, default N"
    )
    parser.add_argument(
        "-bs", "--bytesize", type=int, default=8, help="Bytesize, default 8"
    )
    parser.add_argument(
        "-sb", "--stopbits", type=int, default=1, help="Stopbits, default 1"
    )
    parser.add_argument(
        "-t", "--timeout", type=float, default=2, help="Serial timeout, default 2"
    )
    parser.add_argument(
        "-cs", "--chunk-size", type=int, default=1024, help="Chunk size, default 1024"
    )
    parser.add_argument(
        "-x", "--xmodem", action="store_true", help="Force XMODEM protocol"
    )
    parser.add_argument(
        "-g",
        "--ymodem-g",
        action="store_true",
        help="Force YMODEM-G (allowed only for YMODEM)",
    )
...
def get_cli_args():
    ...
    sender_argparser.add_argument(
        "sources", nargs="+", help="Filepaths to send ./filepath.bin ./filepath2.bin"
    )
    ...
    if serial_io.is_open:
        ...
        try:
            ...
            if cmd == "send":
                ...
                logger.info("Waiting for command from Receiver...")
                socket.send(paths, progress_bar.show)
            elif cmd == "recv":
                path = os.path.abspath(dest)
                logger.info("Waiting for response from Sender...")
